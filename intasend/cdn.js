...
})()(
	{
			...
			function (require, module, exports) {
				...
				var Shape = function Shape(container, opts) {
					...
					var newOpts = utils.extend(
						{ attachment: undefined, shape: this },
						this._opts
					);
		...
		10: [
			function (require, module, exports) {
				...
				module.exports = { MobileCheck: MobileCheck };
			},
			{},
		],
		11: [
				...
				var IntaSend = function (obj) {
					...
					this._createCheckoutRequest = function (payload) {
						...
						xmlhttp.onerror = function (err) {
							alert(
								'We experienced a problem while initiating your request. Please consider starting the payment process again. If this issue persist, please contact us at support@intasend.com'
							);
							self._hideProcessing();
						};
					...
					this._btnElement = document.getElementsByClassName(
						this._element
					);
					if (this._btnElement) {
					...
					this._prepareModal = function () {
						...
						let modalContent =
							document.createElement('modal-content');
						if (this.is_mobile) {
					...
					this.onClose = function () {
						...
						if (this.layout === 'popup') {
							bindEvent(window, 'message', function (e) {
								if (
									e.origin ===
										'https://websdk-sandbox.intasend.com' ||
									e.origin === 'https://websdk.intasend.com'
								) {
									if (e.data.message) {
										if (
											e.data.message.identitier ==
											'intasend-close-modal-cdrtl'
										) {
											self.exitPay();
										}
									}
					...
					this._prepareFrame = function (
						...
						if (this._live) {
							ifrm.setAttribute(
								'src',
								'https://websdk.intasend.com/?' + params
							);
						} else {
							ifrm.setAttribute(
								'src',
								'https://websdk-sandbox.intasend.com/?' + params
							);
						}
						...
						bindEvent(window, 'message', function (e) {
							if (
								...
								if (e.data.message) {
									if (
										e.data.message.identitier ==
										'intasend-status-update-cdrtl'
									) {
										if (
											e.data.message.state === eventName
										) {
